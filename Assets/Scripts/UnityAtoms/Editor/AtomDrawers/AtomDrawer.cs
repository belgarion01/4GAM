...
namespace UnityAtoms
{
                    ...
                    if (GUI.Button(firstButtonRect, "✓"))
                    {
                        if (_nameOfNewAtom.Length > 0)
                        {
                            try
                            {
                                ...
                                string path = AtomsSettings.Instance.atomCreatePath;
                                Debug.Log(path);
                                AssetDatabase.CreateAsset(so, path + _nameOfNewAtom + ".asset");
    ...
    [CustomPropertyDrawer(typeof(QuaternionQuaternionEvent))]
    public class QuaternionQuaternionEventDrawer : AtomDrawer<QuaternionQuaternionEvent> { }
    ...
    public class QuaternionVariableVariableDrawer : AtomDrawer<QuaternionVariable> { }
}
#endif
